...
export async function GET(
  ...
  try {
    ...
    const uniqueVoterIds = Array.from(
      new Set(votingSession.casts.map((c) => c.userId)),
    );
        ...
        <Page size="A4" style={styles.page}>
          ...
          <View style={styles.header}>
            ...
            <View style={styles.metaGrid}>
              ...
              <View style={styles.metaItem}>
                ...
                <Text style={styles.metaVal}>
                  {votingSession.organization?.name ??
              ...
              <View style={styles.metaItem}>
                ...
                <Text style={styles.metaVal}>
                  {totalVotersCount} propietarios
                </Text>
              </View>
              <View style={styles.metaItem}>
                <Text style={styles.metaLabel}>Ponderación</Text>
                <Text style={styles.metaVal}>
                  {votingSession.coefficientWeighted
                    ? "Por Coeficiente"
                    : "1 voto / vecino"}
                </Text>
              </View>
          ...
          <View style={styles.legalBox}>
            <Text style={styles.legalText}>
              D./Dña.{" "}
              {votingSession.author?.name ??
                "El Administrador de Fincas Colegiado"}
              , en calidad de Secretario-Administrador de la comunidad,
              certifica que se ha procedido al cómputo y escrutinio de los votos
              emitidos telemáticamente conforme a los preceptos de la Ley de
          ...
          <View style={styles.section}>
            <Text style={styles.sectionTitle}>
              {isJunta
                ? "ESCRUTINIO POR PUNTOS DEL ORDEN DEL DÍA"
            ...
            {itemsList.map((item, idx) => {
              ...
              const approveCasts = itemCasts.filter(
                (c) => c.choice === "APPROVE",
              );
              const rejectCasts = itemCasts.filter(
                (c) => c.choice === "REJECT",
              );
              const abstainCasts = itemCasts.filter(
                (c) => c.choice === "ABSTAIN",
              );

              const approveWeight = approveCasts.reduce(
                (s, c) => s + (c.coefficient || 1),
                0,
              );
              const rejectWeight = rejectCasts.reduce(
                (s, c) => s + (c.coefficient || 1),
                0,
              );
              const abstainWeight = abstainCasts.reduce(
                (s, c) => s + (c.coefficient || 1),
                0,
              );
              ...
              const approvePct =
                totalWeight > 0
                  ? ((approveWeight / totalWeight) * 100).toFixed(1)
                  : "0.0";
              const rejectPct =
                totalWeight > 0
                  ? ((rejectWeight / totalWeight) * 100).toFixed(1)
                  : "0.0";
              const abstainPct =
                totalWeight > 0
                  ? ((abstainWeight / totalWeight) * 100).toFixed(1)
                  : "0.0";
              ...
              return (
                <View key={item.id} style={styles.pointCard}>
                  <View style={styles.pointHeader}>
                    ...
                    {item.budget ? (
                      <Text style={styles.pointBudget}>{item.budget}</Text>
                    ) : null}
                  </View>
          ...
          <View style={styles.section}>
            <Text style={styles.sectionTitle}>
              REGISTRO Y TRAZABILIDAD DE VOTOS
            </Text>
              ...
              votingSession.casts.map((cast) => {
                ...
                return (
                  <View key={cast.id} style={styles.tableRow}>
                    ...
                    <Text style={[styles.colVoto, { color: choiceColor }]}>
                      {choiceText}
                    </Text>
                    <Text style={styles.colCoef}>
                      {(cast.coefficient || 1).toFixed(2)}%
                    </Text>
                  </View>
                );
              })
          ...
          <View style={styles.signatures}>
            <View style={styles.sigBox}>
              <Text style={styles.sigTitle}>
                Firma del Secretario-Administrador
              </Text>
            ...
            <View style={styles.sigBox}>
              <Text style={styles.sigTitle}>
                Vº Bº El Presidente de la Comunidad
              </Text>
          ...
          <Text style={styles.footer}>
            Acta generada y custodiada electrónicamente por Aconvi el{" "}
            {new Date().toLocaleDateString("es-ES")} · Validez legal acreditada
